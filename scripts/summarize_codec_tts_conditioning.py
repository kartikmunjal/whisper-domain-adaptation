...
def main():
    ...
    result={"schema_version":1,"n_trials":5,"seeds":[11,22,33,44,55],"shuffled_minus_true_nll_mean_ci95":nll,"generated_sequence_sensitivity_mean_ci95":sensitivity,"conditioning_broken_by_locked_gate":conditioning_broken,"position_error_counts":errors.tolist(),"position_event_counts":events.tolist(),"position_error_rates":rates.tolist(),"attention_centroid_r_mean_ci95":attn_r,"attention_entropy_mean_ci95":attn_entropy,"input_reports":[str(Path(a.input_dir)/f"seed_{s}.json") for s in (11,22,33,44,55)]}
