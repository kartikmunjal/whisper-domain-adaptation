...
foreach ($seed in @(11, 22, 33, 44, 55)) {
  $generation = "experiments/results/codec_tts_corrective/seed_$seed"
  if (-not (Test-Path "$generation/generation_report.json")) {
    Invoke-CheckedPython scripts/synthesize_codec_tts.py `
      --tts-checkpoint "checkpoints/codec_tts_corrective/seed_$seed/model.pt" `
      --seed $seed `
...
Invoke-CheckedPython scripts/summarize_codec_tts.py `
  --results-dir experiments/results/codec_tts_corrective `
  --training-dir checkpoints/codec_tts_corrective `
  --output experiments/results/codec_tts_corrective/summary.json `
  --markdown-output experiments/results/codec_tts_corrective/summary.md
